...
| `rounds.scriptEnvironment` | `[]` | Environment variables `runScript` may pass on despite looking like credentials — see [below](#passing-a-token-to-a-command). |
...
### Passing a token to a command

A command spawned by `runScript` gets the editor's environment with the credential-looking parts
removed: any variable whose name contains `token`, `secret`, `password`, `key` or `credential`. Some
commands need exactly one of those, so name it:
...
Two things have to be true for this to work. The variable must be named here, and it must be in the
editor's own environment in the first place: variables exported from `.zshrc` or `.bashrc` reach it
when the editor is started from a terminal, or through the editor's own login shell resolution. If a
command still cannot see the variable, check that the editor can — a run cannot pass on what the
editor never had.
...
| A command cannot see a token from `.zshrc` | Name the variable in `rounds.scriptEnvironment`, and make sure the editor itself has it: start the editor from a terminal if its environment does not come from your login shell. |
