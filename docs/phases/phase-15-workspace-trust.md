...
- `workspaceTrusted` is optional on `ToolContext` and absent means trusted, so every existing
  caller keeps working and only the one that matters — the runner — states it explicitly. It is
  read per run rather than captured at activation: trust can be granted while a window is open.
...
- [x] `package.json` declares limited untrusted-workspace support and restricts
      `rounds.scriptWhitelist`.
- [x] With trust withheld, `runScript` never executes, and the run continues with a recorded
      denial rather than a failure.
- [x] Reading files and file prompts still work, and the reasoning is documented in this phase
      and pinned by a test.
- [x] An agent that depends on `runScript` is shown as not ready before it is due, with the reason.
- [x] Check Setup still reports six checks.
