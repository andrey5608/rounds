...
### 17.5 Write tools — out of scope, and what it would take
...
- [ ] `plan.md` and `docs/leftovers.md` say a read-only tool is in scope and writing from chat
      is not.
- [ ] `rounds_query` answers the six kinds from live state, and rejects unknown fields by name.
- [ ] No tool result can contain a token, proven by a test that plants one in the state.
- [ ] The consent gate is untouched: `check-consent-gate.mjs` still reports one model call site.
- [ ] The manifest declares exactly one tool, and no code path in it writes to the store.
